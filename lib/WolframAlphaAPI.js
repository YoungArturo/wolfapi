 ...
 */
function createApiParams(baseUrl, input, output = 'string') { //This may need to be changed to output = image
  return new Promise((resolve, reject) => {
    switch (typeof input) {
      case 'string':
        resolve({ url: `${baseUrl}&i=${urlencode(input)}`, output });
        break;
      case 'object':
...
/* Commented bc it is not getting used
  getSimple(input) {
   ...
   */
   ...
   /* Commented since it doesn't get used
  getShort(input) {
  ...
  */ 
  ...
  /*Commented bc it won't get used
  getSpoken(input) {
